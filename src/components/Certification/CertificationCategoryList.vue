<template>
  ...
  <q-page padding>
    ...
    <div v-for="{ scheme, rows } in store.rowsByScheme" :key="scheme.id" class="q-mb-lg">
      <div class="scheme-header">{{ scheme.description }}</div>
      <q-table
        :grid="$q.screen.xs"
        flat
        bordered
          ...
          <q-tr v-else :props="props" class="cert-row-product">
            ...
            <q-td key="fee" :props="props" class="text-right">
              ...
              <div class="text-caption text-grey-7">
                <em>(Total: {{ formatFee(props.row.totalFee!) }})</em>
              </div>
            </q-td>
            <q-td key="actions" :props="props">
              <q-btn flat round dense icon="edit" @click="dialogRef?.openEdit(props.row)">
...
function standardLabel(id: number): string {
  const s = standardStore.latestStandards.find((s) => s.id === id);
  return s ? `${s.number}${s.revision ? ` (${s.revision})` : ''}` : `#${id}`;
}
